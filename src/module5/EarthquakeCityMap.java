...
public class EarthquakeCityMap extends PApplet {
	...
	private void selectMarkerIfHover(List<Marker> markers) {
		...
		/*
		 * markers.forEach(m -> m.setSelected(false));
		 */
		 ...
		 * markers.forEach(m -> { m.setSelected(false);
		 * 
		 * if (m.isInside(map, mouseX, mouseY)) { m.setSelected(true); lastSelected =
		 * (CommonMarker) m; return; } });
		 */
	...
	public void mousePressed() {
		if (lastSelected instanceof EarthquakeMarker) {
			...
			float magnitude = marker.getMagnitude();
			float radius = 20 * (1.8f * (2 * magnitude - 5));
			...
			for (Marker m : cityMarkers) {
				float distance = (float) Math.sqrt(Math.pow(m.getLocation().x - marker.getLocation().x, 2)
						+ Math.pow(m.getLocation().y - marker.getLocation().y, 2));
