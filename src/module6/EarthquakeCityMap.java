...
public class EarthquakeCityMap extends PApplet {
	...
	/*
	 * Shows count for the number of nearby earthquakes (within threatCircle),
	 * the average magnitude and the strongest earthquakes's coordinates.
	 */
	public void showMenu() {
		if (lastClicked != null && lastClicked instanceof CityMarker) {
			...
			EarthquakeMarker mostStrong = null;
			...
			for (Marker m : quakeMarkers) {
				...
				if (quakeMarker.getDistanceTo(lastClicked.getLocation()) <= quakeMarker.threatCircle()) {
					...
					if (mostStrong == null) {
						mostStrong = quakeMarker;
					} else {
						if (quakeMarker.getMagnitude() > mostStrong.getMagnitude()) {
							mostStrong = quakeMarker;
						}
					}
				}
			...
			textSize(12);
			text("Amount earthquakes", xbase + 6, ybase + 25);
